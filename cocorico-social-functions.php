...
if(!function_exists('coco_social_facebook_count')){
	function coco_social_facebook_count($url , $id){
		...
		if(get_transient($key) === false){
                        ...
                        $url = 'https://graph.facebook.com/v2.8?fields=share&'.$fbToken.'&id='.$url;
			...
			if ( !is_wp_error( $response ) ) {
				...
				$count = isset($response['share']['share_count']) ? coco_social_convert_count($response['share']['share_count']) : '0';
